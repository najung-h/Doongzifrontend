...
- **분석 중 (Loading UI)**: 
  - 프로그레스 바 또는 로딩 스피너
  - "계약서를 꼼꼼히 읽고 있어요... (약 30~60초 소요)" 문구 표시
  - **주의**: 분석 시간이 길어질 수 있으므로 사용자가 이탈하지 않도록 시각적 피드백 제공 필수
- **분석 결과 (성공 시 즉시 표시)**:
  - 분석 결과 모달 또는 결과 페이지로 전환
  - 위험 등급: 낮음(초록), 보통(노랑), 높음(빨강)
  - 요약(Summary) 및 발견된 문제점(Issues) 리스트
  - [체크리스트에 자동 반영하기] 버튼

#### 프로세스 흐름
1. **파일 업로드**: 드래그 앤 드롭으로 파일(PDF) 선택.
2. **동기식 분석 요청**: 서버로 파일 전송 및 실시간 분석 대기 (최대 2분 소요 가능).
3. **결과 수신 및 렌더링**: 
   - 서버로부터 분석 결과(JSON)를 즉시 수신.
   - 화면에 위험 등급과 분석 요약을 렌더링.
   - (선택) 분석 상세 리포트는 이메일로도 자동 발송됨을 안내.
4. **체크리스트 연동**: 분석 결과에 포함된 `autoCheckItems` 데이터를 기반으로 `ChecklistPage`의 해당 항목들을 자동 완료 처리.
